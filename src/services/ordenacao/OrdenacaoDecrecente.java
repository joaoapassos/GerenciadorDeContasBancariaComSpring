package services.ordenacao;

import java.util.ArrayList;
import java.util.Comparator;
import java.util.List;

import interfaces.OrdenacaoInterface;
import model.contas.Conta;

/**
 *
 * @author joaoapassos
 */

public class OrdenacaoDecrecente implements OrdenacaoInterface{
    
    @Override
    public List<Conta> aplicar(List<Conta> contas) {
        Comparator<Conta> ordenacao = (c1, c2) -> c2.getSaldo().compareTo(c1.getSaldo());
        List<Conta> ordenados = new ArrayList<>(contas);
        ordenados.sort(ordenacao);
        return ordenados;
    }
}
